...
(function() {
  ...
  function D2(f, a) {
    ...
    for (e = w2; 0 <= f.l(a); ) {
      ...
      g2 = v2(c);
      for (b2 = g2.j(a); B2(b2) || 0 < b2.l(f); )
        c -= d, g2 = v2(c), b2 = g2.j(a);
      C2(g2) && (g2 = z2);
      e = e.add(g2);
      f = F2(f, b2);
    }
  ...
  function J2(f, a) {
    var c = a >> 5;
    a %= 32;
    for (var d = f.g.length - c, e = [], g2 = 0; g2 < d; g2++)
      e[g2] = 0 < a ? f.i(g2 + c) >>> a | f.i(g2 + c + 1) << 32 - a : f.i(g2 + c);
    return new t(e, f.h);
  }
...
var F = "permission-denied";
var b = "unauthenticated";
...
function __PRIVATE_compareByteArrays(t, e) {
  for (let r = 0; r < t.length && r < e.length; ++r)
    if (t[r] !== e[r])
      return __PRIVATE_primitiveComparator(t[r], e[r]);
  return __PRIVATE_primitiveComparator(t.length, e.length);
}
...
var ResourcePath = class _ResourcePath extends BasePath {
  ...
  static fromString(...t) {
    ...
    for (const r of t) {
      if (r.indexOf("//") >= 0)
        throw new FirestoreError(y, `Invalid segment (${r}). Paths must not contain // in them.`);
...
var FieldPath$1 = class _FieldPath$1 extends BasePath {
  ...
  static fromServerFormat(t) {
    ...
    const __PRIVATE_addCurrentSegment = () => {
      if (0 === r.length)
        throw new FirestoreError(y, `Invalid field path (${t}). Paths must not be empty, begin with '.', end with '.', or contain '..'`);
      e.push(r), r = "";
    };
    let i = false;
    for (; n < t.length; ) {
      ...
      if ("\\" === e2) {
        if (n + 1 === t.length)
          throw new FirestoreError(y, "Path has trailing escape character: " + t);
        const e3 = t[n + 1];
        if ("\\" !== e3 && "." !== e3 && "`" !== e3)
          throw new FirestoreError(y, "Path has invalid escape sequence: " + t);
        r += e3, n += 2;
      } else
        "`" === e2 ? (i = !i, n++) : "." !== e2 || i ? (r += e2, n++) : (__PRIVATE_addCurrentSegment(), n++);
    }
    if (__PRIVATE_addCurrentSegment(), i)
      throw new FirestoreError(y, "Unterminated ` in path: " + t);
...
function __PRIVATE_validateNonEmptyArgument(t, e, r) {
  if (!r)
    throw new FirestoreError(y, `Function ${t}() cannot be called with an empty ${e}.`);
}
function __PRIVATE_validateDocumentPath(t) {
  if (!DocumentKey.isDocumentKey(t))
    throw new FirestoreError(y, `Invalid document reference. Document references must have an even number of segments, but ${t} has ${t.length}.`);
}
function __PRIVATE_validateCollectionPath(t) {
  if (DocumentKey.isDocumentKey(t))
    throw new FirestoreError(y, `Invalid collection reference. Collection references must have an odd number of segments, but ${t} has ${t.length}.`);
}
...
function __PRIVATE_cast(t, e) {
  ...
  (t = t._delegate), !(t instanceof e)) {
    if (e.name === t.constructor.name)
      throw new FirestoreError(y, "Type does not match the expected instance. Did you pass a reference from a different Firestore SDK?");
    {
...
function __PRIVATE_validatePositiveNumber(t, e) {
  if (e <= 0)
    throw new FirestoreError(y, `Function ${t}() requires a positive number, but it was: ${e}.`);
}
...
var __PRIVATE_RestConnection = class {
  ...
  I(t, e, r, n, i) {
    ...
    return this.F(t, o, a, r, _).then((e2) => (__PRIVATE_logDebug(M, `Received RPC '${t}' ${s}: `, e2), e2), (e2) => {
      throw __PRIVATE_logWarn(M, `RPC '${t}' ${s} failed with error: `, e2, "url: ", o, "request:", r), e2;
    });
  }
  D(t, e, r, n, i, s) {
...
function __PRIVATE_mapCodeFromHttpStatus(t) {
  if (void 0 === t)
    return __PRIVATE_logError("RPC_ERROR", "HTTP error has no status"), p;
  switch (t) {
    case 200:
      return V;
    case 400:
...
function __PRIVATE_validateJSON(t, e) {
  if (!__PRIVATE_isPlainObject(t))
    throw new FirestoreError(y, "JSON must be an object");
  let r;
  for (const n in e)
    if (e[n]) {
...
var Timestamp = class _Timestamp {
  ...
  constructor(t, e) {
    if (this.seconds = t, this.nanoseconds = e, e < 0)
      throw new FirestoreError(y, "Timestamp nanoseconds out of range: " + e);
    if (e >= 1e9)
      throw new FirestoreError(y, "Timestamp nanoseconds out of range: " + e);
    if (t < K)
      throw new FirestoreError(y, "Timestamp seconds out of range: " + t);
    if (t >= 253402300800)
      throw new FirestoreError(y, "Timestamp seconds out of range: " + t);
  }
  /**
   ...
   */
  static fromJSON(t) {
    if (__PRIVATE_validateJSON(t, _Timestamp._jsonSchema))
      return new _Timestamp(t.seconds, t.nanoseconds);
  }
  /**
...
function __PRIVATE_typeOrder(t) {
  return "nullValue" in t ? 0 : "booleanValue" in t ? 1 : "integerValue" in t || "doubleValue" in t ? 2 : "timestampValue" in t ? 3 : "stringValue" in t ? 5 : "bytesValue" in t ? 6 : "referenceValue" in t ? 7 : "geoPointValue" in t ? 8 : "arrayValue" in t ? 9 : "mapValue" in t ? __PRIVATE_isServerTimestamp(t) ? 4 : (
    ...
    function __PRIVATE_isMaxValue(t2) {
      return (((t2.mapValue || {}).fields || {}).__type__ || {}).stringValue === H;
    }(t) ? 9007199254740991 : (
      /** Returns true if `value` is a VetorValue. */
      function __PRIVATE_isVectorValue(t2) {
        var e, r;
        const n = null === (r = ((null === (e = null == t2 ? void 0 : t2.mapValue) || void 0 === e ? void 0 : e.fields) || {})[J]) || void 0 === r ? void 0 : r.stringValue;
        return n === Z;
      }(t) ? 10 : 11
    )
  ) : fail(28295, {
...
function __PRIVATE_queryNormalizedOrderBy(t) {
  ...
  if (null === e.q) {
    ...
    for (const r2 of e.explicitOrderBy)
      e.q.push(r2), t2.add(r2.field.canonicalString());
...
function fromName(t, e) {
  ...
  if (r.get(1) !== t.databaseId.projectId)
    throw new FirestoreError(y, "Tried to deserialize key from different project: " + r.get(1) + " vs " + t.databaseId.projectId);
  if (r.get(3) !== t.databaseId.database)
    throw new FirestoreError(y, "Tried to deserialize key from different database: " + r.get(3) + " vs " + t.databaseId.database);
...
var __PRIVATE_DatastoreImpl = class extends Datastore {
  ...
  rt() {
    if (this.et)
      throw new FirestoreError(S, "The client has already been terminated.");
  }
  ...
  I(t, e, r, n) {
    return this.rt(), Promise.all([this.authCredentials.getToken(), this.appCheckCredentials.getToken()]).then(([i, s]) => this.connection.I(t, __PRIVATE_toResourcePath(e, r), n, i, s)).catch((t2) => {
      throw "FirebaseError" === t2.name ? (t2.code === b && (this.authCredentials.invalidateToken(), this.appCheckCredentials.invalidateToken()), t2) : new FirestoreError(p, t2.toString());
    });
  }
...
var FirestoreSettingsImpl = class {
  constructor(t) {
    ...
    if (void 0 === t.host) {
      if (void 0 !== t.ssl)
        throw new FirestoreError(y, "Can't provide ssl option if host option is not set");
      this.host = ot, this.ssl = at;
    } else
      this.host = t.host, this.ssl = null !== (e = t.ssl) && void 0 !== e ? e : at;
    if (this.isUsingEmulator = void 0 !== t.emulatorOptions, this.credentials = t.credentials, this.ignoreUndefinedProperties = !!t.ignoreUndefinedProperties, this.localCache = t.localCache, void 0 === t.cacheSizeBytes)
      this.cacheSizeBytes = 41943040;
    else {
      if (-1 !== t.cacheSizeBytes && t.cacheSizeBytes < st)
        throw new FirestoreError(y, "cacheSizeBytes must be at least 1048576");
      this.cacheSizeBytes = t.cacheSizeBytes;
    }
    !function __PRIVATE_validateIsNotUsedTogether(t2, e2, r2, n) {
      if (true === e2 && true === n)
        throw new FirestoreError(y, `${t2} and ${r2} cannot be used together.`);
      ...
      if (void 0 !== t2.timeoutSeconds) {
        if (isNaN(t2.timeoutSeconds))
          throw new FirestoreError(y, `invalid long polling timeout: ${t2.timeoutSeconds} (must not be NaN)`);
        if (t2.timeoutSeconds < 5)
          throw new FirestoreError(y, `invalid long polling timeout: ${t2.timeoutSeconds} (minimum allowed value is 5)`);
        if (t2.timeoutSeconds > 30)
          throw new FirestoreError(y, `invalid long polling timeout: ${t2.timeoutSeconds} (maximum allowed value is 30)`);
      }
...
var Firestore = class {
  ...
  get app() {
    if (!this._app)
      throw new FirestoreError(S, "Firestore was not initialized using the Firebase SDK. 'app' is not available");
    return this._app;
  }
  get _initialized() {
  ...
  _setSettings(t) {
    if (this._settingsFrozen)
      throw new FirestoreError(S, "Firestore has already been started and its settings can no longer be changed. You can only modify settings before calling any other methods on a Firestore object.");
...
function initializeFirestore(t, e, r) {
  ...
  if (n.isInitialized(r))
    throw new FirestoreError(S, "Firestore can only be initialized once per app.");
  return n.initialize({
    options: e,
...
function connectFirestoreEmulator(t, e, r, n = {}) {
  ...
  if (!deepEqual(_, a) && (t._setSettings(_), n.mockUserToken)) {
    ...
    if ("string" == typeof n.mockUserToken)
      ...
      if (!s2)
        throw new FirestoreError(y, "mockUserToken must contain 'sub' or 'user_id' field!");
      r2 = new User(s2);
    }
...
var DocumentReference = class _DocumentReference {
  ...
  static fromJSON(t, e, r) {
    if (__PRIVATE_validateJSON(e, _DocumentReference._jsonSchema))
      return new _DocumentReference(t, r || null, new DocumentKey(ResourcePath.fromString(e.referencePath)));
  }
};
...
function collection(t, e, ...r) {
  ...
  }
  {
    if (!(t instanceof DocumentReference || t instanceof CollectionReference))
      throw new FirestoreError(y, "Expected first argument to collection() to be a CollectionReference, a DocumentReference or FirebaseFirestore");
...
function collectionGroup(t, e) {
  if (t = __PRIVATE_cast(t, Firestore), __PRIVATE_validateNonEmptyArgument("collectionGroup", "collection id", e), e.indexOf("/") >= 0)
    throw new FirestoreError(y, `Invalid collection ID '${e}' passed to function collectionGroup(). Collection IDs must not contain '/'.`);
  return new Query(
    t,
    /* converter= */
    null,
...
var Bytes = class _Bytes {
   ...
   */
  static fromJSON(t) {
    if (__PRIVATE_validateJSON(t, _Bytes._jsonSchema))
      return _Bytes.fromBase64String(t.bytes);
  }
};
...
var FieldPath = class {
  ...
  constructor(...t) {
    for (let e = 0; e < t.length; ++e)
      if (0 === t[e].length)
        throw new FirestoreError(y, "Invalid field name at argument $(i + 1). Field names must not be empty.");
...
var GeoPoint = class _GeoPoint {
  ...
  constructor(t, e) {
    if (!isFinite(t) || t < -90 || t > 90)
      throw new FirestoreError(y, "Latitude must be a number between -90 and 90, but was: " + t);
    if (!isFinite(e) || e < -180 || e > 180)
      throw new FirestoreError(y, "Longitude must be a number between -180 and 180, but was: " + e);
   ...
   */
  static fromJSON(t) {
    if (__PRIVATE_validateJSON(t, _GeoPoint._jsonSchema))
      return new _GeoPoint(t.latitude, t.longitude);
  }
};
...
var VectorValue = class _VectorValue {
  ...
  static fromJSON(t) {
    if (__PRIVATE_validateJSON(t, _VectorValue._jsonSchema)) {
      if (Array.isArray(t.vectorValues) && t.vectorValues.every((t2) => "number" == typeof t2))
        return new _VectorValue(t.vectorValues);
...
var __PRIVATE_ParseContextImpl = class ___PRIVATE_ParseContextImpl {
  ...
  ct(t) {
    if (0 === t.length)
      throw this.ft("Document fields must not be empty");
    if (__PRIVATE_isWrite(this.it) && ut.test(t))
      throw this.ft('Document fields cannot begin and end with "__"');
  }
};
...
function __PRIVATE_parseSetData(t, e, r, n, i, s = {}) {
  ...
  if (s.merge)
    ...
    for (const n2 of s.mergeFields) {
      ...
      if (!o.contains(i2))
        throw new FirestoreError(y, `Field '${i2}' is specified in your field mask but missing from your input data.`);
...
var __PRIVATE_DeleteFieldValueImpl = class ___PRIVATE_DeleteFieldValueImpl extends FieldValue {
  _toFieldTransform(t) {
    if (2 !== t.it)
      throw 1 === t.it ? t.ft(`${this._methodName}() can only appear at the top level of your update data`) : t.ft(`${this._methodName}() cannot be used with set() unless you pass {merge:true}`);
...
function __PRIVATE_parseUpdateVarargs(t, e, r, n, i, s) {
  ...
  if (s.length % 2 != 0)
    throw new FirestoreError(y, `Function ${e}() needs to be called with an even number of arguments that alternate between field names and values.`);
...
function __PRIVATE_parseData(t, e) {
  ...
  if (t instanceof FieldValue)
    return function __PRIVATE_parseSentinelFieldValue(t2, e2) {
      if (!__PRIVATE_isWrite(e2.it))
        throw e2.ft(`${t2._methodName}() can only be used with update() and set()`);
      if (!e2.path)
        throw e2.ft(`${t2._methodName}() is not currently supported inside arrays`);
  ...
  if (
    ...
    if (e.settings._t && 4 !== e.it)
      throw e.ft("Nested arrays are not supported");
  ...
  return function __PRIVATE_parseScalarValue(t2, e2) {
    ...
    if (t2 instanceof DocumentReference) {
      ...
      if (!n.isEqual(r))
        throw e2.ft(`Document reference is for database ${n.projectId}/${n.database} but should be for database ${r.projectId}/${r.database}`);
      return {
            ...
            [X]: {
              arrayValue: {
                values: t3.toArray().map((t4) => {
                  if ("number" != typeof t4)
                    throw e3.ft("VectorValues must only contain numeric values.");
...
function query(t, e, ...r) {
  ...
  e instanceof AppliableConstraint && n.push(e), n = n.concat(r), function __PRIVATE_validateQueryConstraintArray(t2) {
    ...
    if (e2 > 1 || e2 > 0 && r2 > 0)
      throw new FirestoreError(y, "InvalidQuery. When using composite filters, you cannot use more than one filter at the top level. Consider nesting the multiple filters within an `and(...)` statement. For example: change `query(query, where(...), or(...))` to `query(query, and(where(...), or(...)))`.");
  }(n);
  for (const e2 of n)
    t = e2._apply(t);
  return t;
    ...
    const e = __PRIVATE_newUserDataReader(t.firestore), r = function __PRIVATE_newQueryFilter(t2, e2, r2, n, i, s, o) {
      ...
      if (i.isKeyField()) {
        if ("array-contains" === s || "array-contains-any" === s)
          throw new FirestoreError(y, `Invalid Query. You can't perform '${s}' queries on documentId().`);
        if ("in" === s || "not-in" === s) {
...
var QueryOrderByConstraint = class _QueryOrderByConstraint extends QueryConstraint {
  ...
  _apply(t) {
    const e = function __PRIVATE_newQueryOrderBy(t2, e2, r) {
      if (null !== t2.startAt)
        throw new FirestoreError(y, "Invalid query. You must not call startAt() or startAfter() before calling orderBy().");
      if (null !== t2.endAt)
        throw new FirestoreError(y, "Invalid query. You must not call endAt() or endBefore() before calling orderBy().");
...
function __PRIVATE_newQueryBoundFromDocOrFields(t, e, r, n) {
  if (r[0] = getModularInstance(r[0]), r[0] instanceof DocumentSnapshot)
    return function __PRIVATE_newQueryBoundFromDocument(t2, e2, r2, n2, i) {
      if (!n2)
        throw new FirestoreError(w, `Can't use a DocumentSnapshot that doesn't exist for ${r2}().`);
      const s = [];
      for (const r3 of __PRIVATE_queryNormalizedOrderBy(t2))
        if (r3.field.isKeyField())
          ...
          if (__PRIVATE_isServerTimestamp(t3))
            throw new FirestoreError(y, 'Invalid query. You are trying to start or end a query using a document for which the field "' + r3.field + '" is an uncommitted server timestamp. (Since the value of this field is unknown, you cannot start/end a query with it.)');
          if (null === t3) {
    ...
    }(t._query, t.firestore._databaseId, e, r[0]._document, n);
  {
    ...
    return function __PRIVATE_newQueryBoundFromFields(t2, e2, r2, n2, i2, s) {
      ...
      if (i2.length > o.length)
        throw new FirestoreError(y, `Too many arguments provided to ${n2}(). The number of arguments must be less than or equal to the number of orderBy() clauses`);
      const a = [];
      for (let s2 = 0; s2 < i2.length; s2++) {
        const u = i2[s2];
        if (o[s2].field.isKeyField()) {
          if ("string" != typeof u)
            throw new FirestoreError(y, `Invalid query. Expected a string for document ID in ${n2}(), but got a ${typeof u}`);
          if (!__PRIVATE_isCollectionGroupQuery(t2) && -1 !== u.indexOf("/"))
            throw new FirestoreError(y, `Invalid query. When querying a collection and ordering by documentId(), the value passed to ${n2}() must be a plain document ID, but '${u}' contains a slash.`);
          const r3 = t2.path.child(ResourcePath.fromString(u));
          if (!DocumentKey.isDocumentKey(r3))
            throw new FirestoreError(y, `Invalid query. When querying a collection group and ordering by documentId(), the value passed to ${n2}() must result in a valid document path, but '${r3}' is not because it contains an odd number of segments.`);
...
function __PRIVATE_parseDocumentIdValue(t, e, r) {
  if ("string" == typeof (r = getModularInstance(r))) {
    if ("" === r)
      throw new FirestoreError(y, "Invalid query. When querying with documentId(), you must provide a valid document ID, but it was an empty string.");
    if (!__PRIVATE_isCollectionGroupQuery(e) && -1 !== r.indexOf("/"))
      throw new FirestoreError(y, `Invalid query. When querying a collection by documentId(), you must provide a plain document ID, but '${r}' contains a '/' character.`);
    const n = e.path.child(ResourcePath.fromString(r));
    if (!DocumentKey.isDocumentKey(n))
      throw new FirestoreError(y, `Invalid query. When querying a collection group by documentId(), the value provided must result in a valid document path, but '${n}' is not because it has an odd number of segments (${n.length}).`);
...
function __PRIVATE_validateDisjunctiveFilterElements(t, e) {
  if (!Array.isArray(t) || 0 === t.length)
    throw new FirestoreError(y, `Invalid Query. A non-empty array is required for '${e.toString()}' filters.`);
}
...
function __PRIVATE_validateQueryFilterConstraint(t, e) {
  if (!(e instanceof QueryFieldFilterConstraint || e instanceof QueryCompositeFilterConstraint))
    throw new FirestoreError(y, `Function ${t}() requires AppliableConstraints created with a call to 'where(...)', 'or(...)', or 'and(...)'.`);
}
...
function getDocs(t) {
  (function __PRIVATE_validateHasExplicitOrderByForLimitToLast(t2) {
    if ("L" === t2.limitType && 0 === t2.explicitOrderBy.length)
      throw new FirestoreError(O, "limitToLast() queries require specifying at least one orderBy() clause");
...
var WriteBatch = class {
  ...
  _verifyNotCommitted() {
    if (this._committed)
      throw new FirestoreError(S, "A write batch can no longer be used after commit() has been called.");
  }
};
function __PRIVATE_validateReference(t, e) {
  if ((t = getModularInstance(t)).firestore !== e)
    throw new FirestoreError(y, "Provided document reference is from a different Firestore instance.");
  return t;
}
...
var Transaction$1 = class {
  ...
  async lookup(t) {
    if (this.ensureCommitNotCalled(), this.mutations.length > 0)
      throw this.lastTransactionError = new FirestoreError(y, "Firestore transactions require all reads to be executed before all writes."), this.lastTransactionError;
...
var __PRIVATE_TransactionRunner = class {
  ...
  vt(t) {
    this.Vt > 0 && this.Ft(t) ? (this.Vt -= 1, this.asyncQueue.enqueueAndForget(() => (this.gt(), Promise.resolve()))) : this.deferred.reject(t);
  }
  Ft(t) {
...
var __PRIVATE_AsyncQueueImpl = class {
  constructor(t = Promise.resolve()) {
    this.bt = [], // Is this AsyncQueue being shut down? Once it is set to true, it will not
    // be changed again.
    this.Dt = false, // Operations scheduled to be queued in the future. Operations are
    ...
    this.Bt = () => {
      ...
      t2 && __PRIVATE_logDebug(lt, "Visibility state changed to " + t2.visibilityState), this.It.tt();
    }, this.$t = t;
    const e = getDocument();
  ...
  enqueue(t) {
    ...
    return this.kt(() => this.Dt && this.Ot ? Promise.resolve() : (t().then(e.resolve, e.reject), e.promise)).then(() => e.promise);
  }
  enqueueRetryable(t) {
  ...
  async Lt() {
    if (0 !== this.bt.length) {
      ...
      this.bt.length > 0 && // If there are additional operations, we re-schedule `retryNextOp()`.
  ...
  enqueueAfterDelay(t, e, r) {
    ...
    const n = DelayedOperation.createAndSchedule(this, t, e, r, (t2) => this.xt(t2));
...
function runTransaction(t, e, r) {
  ...
  !function __PRIVATE_validateTransactionOptions(t2) {
    if (t2.maxAttempts < 1)
      throw new FirestoreError(y, "Max attempts must be at least 1");
  }(i);
  ...
  !function __PRIVATE_setSDKVersion(t) {
    ...
    const n = t.getProvider("app").getImmediate(), i = new Firestore(new __PRIVATE_LiteAuthCredentialsProvider(t.getProvider("auth-internal")), new __PRIVATE_LiteAppCheckTokenProvider(n, t.getProvider("app-check-internal")), function __PRIVATE_databaseIdFromApp(t2, e2) {
      if (!Object.prototype.hasOwnProperty.apply(t2.options, ["projectId"]))
        throw new FirestoreError(y, '"projectId" not provided in firebase.initializeApp.');
